...
from interface_generation import confparser  # Author: Tim Dorssers
...
def main(core_switch, rack_port_input, excluded_svi_input, svi_nac, bot):
    filenames = []
    if bot:
        core_switch = 'interface_temp/' + core_switch
        rack_port_input = rack_port_input
    ...
    nac_svi = []
    if not os.path.isfile(core_switch):
        svi_nac = False
    ...
    for key in inventory_matrix:  # Start looping through switches
        ...
        for used_interface in old_config["interface"]:  # For every interface in the old config file.
            print(used_interface + " was returned when looping interfaces in the old config file")
            ...
            if isinstance(old_config["interface"][used_interface]['mode'], list):
                print("Show Running Config is duplicated in the file: " + key + ".log!")
                exit()
            if old_config["interface"][used_interface]["mode"] == "access":  # but only if it's an access interface
                print(used_interface + " is an access interface")
                ...
                print(used_interface + " moves to " + target_interface)
        ...
        print(capacity)
        print(used_interfaces)
        all_interfaces = []
        with open("interface_generation/logic_templates/all_interfaces_48.j2") as f:
        ...
        filename = inventory_matrix[key] + '-' + today.strftime("%d%b%Y%-H%M%S") + '.txt'
...
def templater(input_lst, interface_type):
    with open("interface_generation/configs/interface_" + interface_type + ".j2", "r") as interface_file:
    ...
    for interface in core_config['interface']:
        if 'Vlan' in interface and 'shutdown' not in core_config['interface'][interface]:
            ...
            if core_config['interface'][interface]['ipv4']:
                ...
                print(interface + ' has no IP address configured on this active svi, ignoring this SVI')
...
def config_parse(file):
    if not os.path.exists(file):
        return str(file + ": This file does not exist in the directory!")
    dissector = confparser.Dissector.from_file('interface_generation/ios.yaml')
...
def core_port_workbook_reader(wb_input, cp_wb_input):
    ...
    cp_wb_input = cp_wb_input.replace('interface_temp/','')
...
def port_availability_workbook_reader(wb_input):
    ...
    while e_cell < 10000:
        ...
        if sheet["E" + str(e_cell)].value == "Switch Name: ":
            old_sw = 'interface_temp/' + sheet["H" + str(e_cell)].value
            new_sw = 'interface_temp/' + sheet["Y" + str(e_cell)].value
...
if __name__ == "__main__":
    ...
    main(core_file, rack_port_xl, excluded_svi_lst, SVI_NAC_option, bot)
    run_time = time.time() - start_time
    run_time = round(run_time, 2)
    print("Time to run: " + str(run_time) + " seconds")
